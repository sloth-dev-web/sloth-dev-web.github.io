...
// Exact port of MainScreen constants from GameState.cpp
const SharpRatio = 0.65;
const KBPercent = 0.25;
const KeyRatio = 0.1775;
const A0 = 21;
const C8 = 108;
const C4 = 60;
const Note = { C: 0, CS: 1, D: 2, DS: 3, E: 4, F: 5, FS: 6, G: 7, GS: 8, A: 9, AS: 10, B: 11 };
function NoteVal(n) { return ((n % 12) + 12) % 12; }
// Exact port of the sharp nudge in GetNoteX / RenderKeys:
// CS / FS shift left, AS / DS shift right, GS stays centered.
function sharpNudge(n) {
    const v = NoteVal(n);
    if (v === Note.CS || v === Note.FS) return -SharpRatio / 5.0;
    if (v === Note.AS || v === Note.DS) return SharpRatio / 5.0;
    return 0;
}
// Exact port of MainScreen::GetNoteX
function getNoteX(iNote, startNote, whiteCX, notesX) {
    const iWhiteKeys = whiteCount(startNote, iNote);
    const fStartX = ((isSharp(startNote) ? 1 : 0) - (isSharp(iNote) ? 1 : 0)) * SharpRatio / 2.0 + (isSharp(iNote) ? sharpNudge(iNote) : 0);
    return notesX + whiteCX * (iWhiteKeys + fStartX);
}
// Exact port of MainScreen::RenderGlobals (screen Y section) + RenderKeys header.
// Pure function so webgl.js, Renderer.js and hit-testing all share one layout.
function computeKeyboardLayout(W, H, startNote, endNote, showKB = true) {
    const iAllWhiteKeys = whiteCount(startNote, endNote + 1);
    const fBuffer = (isSharp(startNote) ? SharpRatio / 2.0 : 0.0) +
                    (isSharp(endNote) ? SharpRatio / 2.0 : 0.0);
    const whiteCX = W / (iAllWhiteKeys + fBuffer);
    // Exact port of the RenderGlobals branch: hidden keyboard gives the notes
    // the full height.
    if (!showKB) {
        return { notesX: 0, notesY: 0, notesCX: W, notesCY: H, keysY: H, keysCY: 0, whiteCX, iAllWhiteKeys,
                 fTransitionCY: 0, fRedCY: 0, fSpacerCY: 0, fTopCY: 0, fNearCY: 0 };
    }
    const fMaxKeyCY = H * KBPercent;
    let fIdealKeyCY = whiteCX / KeyRatio;
    // .95 for the top vs near. 2.0 for the spacer. .93 for the transition and the red. ESTIMATE.
    fIdealKeyCY = (fIdealKeyCY / 0.95 + 2.0) / 0.93;
    const notesCY = Math.floor(H - Math.min(fIdealKeyCY, fMaxKeyCY) + 0.5);
    const keysY = notesCY; // m_fNotesY is 0 in the web port
    const keysCY = H - notesCY;
    const fTransitionCY = Math.max(3.0, Math.floor(keysCY * 0.02 + 0.5));
    const fRedCY = Math.floor(keysCY * 0.05 + 0.5);
    const fSpacerCY = 2.0;
    const fTopCY = Math.floor((keysCY - fSpacerCY - fRedCY - fTransitionCY) * 0.95 + 0.5);
    const fNearCY = keysCY - fSpacerCY - fRedCY - fTransitionCY - fTopCY;
    return { notesX: 0, notesY: 0, notesCX: W, notesCY, keysY, keysCY, whiteCX, iAllWhiteKeys, fTransitionCY, fRedCY, fSpacerCY, fTopCY, fNearCY };
}

// ---- ChannelSettings color system (exact port of ChannelSettings::SetColor) ----
// Local HSV helpers (duplicated from webgl.js to avoid an import cycle).
...
// Port of ChannelSettings::SetColor(iColor, dDark, dVeryDark).
// iColor is a Windows COLORREF (0x00BBGGRR); output ints are ABGR
// (0xAABBGGRR, red in the low byte) to match the WGL pipeline.
function setChannelColor(iColor, dDark, dVeryDark) {
    const R = (iColor >> 0) & 0xFF, G = (iColor >> 8) & 0xFF, B = (iColor >> 16) & 0xFF;
    const [H, S, V] = _rgbToHsv(R, G, B);
    const [dR, dG, dB] = _hsvToRgb(H, S, Math.min(100, V * dDark));
    const [vdR, vdG, vdB] = _hsvToRgb(H, S, Math.min(100, V * dVeryDark));
    return {
        origBGR: iColor,
        primary:  (0xFF << 24) | (B << 16) | (G << 8) | R,
        dark:     (0xFF << 24) | (dB << 16) | (dG << 8) | dR,
        veryDark: (0xFF << 24) | (vdB << 16) | (vdG << 8) | vdR
    };
}
// Port of MainScreen::InitColors
function initKeyboardColors() {
    return {
        background:   setChannelColor(0x00464646, 0.7, 1.3),
        kbBackground: setChannelColor(0x00999999, 0.4, 0.0),
        kbRed:        setChannelColor(0x000D0A98, 0.5, 0.5),
        kbWhite:      setChannelColor(0x00FFFFFF, 0.8, 0.6),
        kbSharp:      setChannelColor(0x00404040, 0.5, 0.0),
        kbBadNote:    setChannelColor(0x00808080, 0.6, 0.2)
    };
}
// Port of ColorChannel(iTrack, iChannel, iColor) for a web track color (0xRRGGBB).
function makeTrackColor(rgb) {
    const r = (rgb >> 16) & 0xFF, g = (rgb >> 8) & 0xFF, b = rgb & 0xFF;
    const [H, S, V] = _rgbToHsv(r, g, b);
    const [dR, dG, dB] = _hsvToRgb(H, S, Math.min(100, V * 0.6));
    const [vdR, vdG, vdB] = _hsvToRgb(H, S, Math.min(100, V * 0.2));
    return {
        primary:  (0xFF << 24) | (b << 16) | (g << 8) | r,
        dark:     (0xFF << 24) | (dB << 16) | (dG << 8) | dR,
        veryDark: (0xFF << 24) | (vdB << 16) | (vdG << 8) | vdR
...
// Total key slots. MIDI only addresses 0-127, but extended files/players go
// past G9, so the map continues chromatically (whites then sharps, same rule
// as the 128 entries above) instead of dropping those notes.
const NUM_KEYS = 256;
const MAX_MIDI = NUM_KEYS - 1;
for (let m = 128; m < NUM_KEYS; m++) if (!isSharp(m)) KeyMap.push(m);
for (let m = 128; m < NUM_KEYS; m++) if (isSharp(m)) KeyMap.push(m);
const MidiToKey = new Array(NUM_KEYS).fill(-1);
for (let ki = 0; ki < NUM_KEYS; ki++) { const midi = KeyMap[ki]; if (midi >= 0 && midi < NUM_KEYS) MidiToKey[midi] = ki; }
...
export { SharpRatio, KBPercent, KeyRatio, A0, C8, C4, Note, NoteVal, sharpNudge, getNoteX, computeKeyboardLayout,
         setChannelColor, initKeyboardColors, makeTrackColor,
         NUM_KEYS, MAX_MIDI,
         KeyMap, MidiToKey, GenKeyX, NoteColors, channelColors, trackColorIdx,
