...
let audioFrozen = false;
let frozenMasterVol = null;
let freezeOp = Promise.resolve();

function muteMasterNow() {
    if (!masterGain || !audioContext) return;
    try {
        if (frozenMasterVol == null) {
            frozenMasterVol = settings.get('masterVolume') / 100;
        }
...
function unmuteMasterNow() {
    if (!masterGain || !audioContext) return;
    try {
        const vol = (frozenMasterVol != null)
            ? frozenMasterVol
            : (settings.get('masterVolume') / 100);
        frozenMasterVol = null;
        masterGain.gain.cancelScheduledValues(audioContext.currentTime);
...
function freezeAudio() {
    audioFrozen = true;
    muteMasterNow();
    if (!audioContext) return freezeOp;
    // Serialize suspend behind any in-flight resume so we never race.
    freezeOp = freezeOp.catch(() => {}).then(() => {
        if (audioFrozen && audioContext && audioContext.state === 'running') {
...
function unfreezeAudio() {
    ...
    // Keep audioFrozen until the context is actually running so
    // playSoundFontNote/playKey cannot fire against a dead clock.
    freezeOp = freezeOp.catch(() => {}).then(() => {
...
function playSoundFontNote(k, velocity = 127, when = null, cutPrevious = true) {
    if (!sf2Voices) return;
    // Hard guard: while the song is frozen, never create new SF voices.
    // Held/look-ahead sources are already scheduled on the frozen clock.
    if (audioFrozen) return;
    if (!audioContext || !window.loadedSoundFont || !window.loadedSoundFont.sf2) return;

    const sf2 = window.loadedSoundFont.sf2;
    const midiNote = keyToMidi(k);
    // SoundFonts only cover standard MIDI (0-127); extended slots stay silent
    // here instead of spamming zone warnings (oscillator keys still sound).
            ...
            try {
                ...
                if (v.gain) {
                    try {
                        ...
                        v.gain.gain.setValueAtTime(immediate ? 0 : v.gain.gain.value, now);
                    } catch (e) {}
                }
                if (immediate) {
                    // Explicit stop time cancels look-ahead starts scheduled in
                    // the future; a bare stop() can leave them audible.
                    try { v.source.stop(now); } catch (e) {
                        try { v.source.stop(); } catch (e2) {}
                    }
                    if (v.filter) v.filter.disconnect();
                    v.gain.disconnect();
                    v.source.disconnect();
...
export { voicePool, activeVoices, sf2AudioBuffers, sf2Voices,
         ...
         freezeAudio, unfreezeAudio, isAudioFrozen,
