...
async function loadLanguage(langCode) {
  ...
  try {
    ...
    if (langCode !== 'en' && !fallbackTranslations) {
      try {
        const enRes = await fetch('lang/en.json');
        if (enRes.ok) fallbackTranslations = await enRes.json();
      } catch (e) {}
    }
    if (langCode === 'en') fallbackTranslations = currentTranslations;
...
function t(key) {
  if (currentTranslations.settings && currentTranslations.settings[key]) return currentTranslations.settings[key];
  if (fallbackTranslations && fallbackTranslations.settings && fallbackTranslations.settings[key]) return fallbackTranslations.settings[key];
  return key;
}
