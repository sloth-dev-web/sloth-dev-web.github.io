...
function parseTrack(r, trackEnd, light = false) {
  ...
  while (r.tell() < trackEnd) {
    ...
    if (statusByte === 0xFF) {
      ...
      switch (type) {
        case 0x8: {  // Note Off
          // No 127 clamp: like Kiva, out-of-range bytes address extended
          // slots (up to 255) instead of being folded onto G9.
...
export function parseMIDI(arrayBuffer, options = {}) {
  ...
  // Track events are tick-ordered and the tempo map is tick-ordered, so one
  // forward segment pointer per track makes this O(events + tempos) instead
  // of scanning the whole tempo map for every event.
  for (const track of tracks) {
    let seg = 0;
    for (const ev of track.events) {
      while (seg + 1 < tempoMap.length && tempoMap[seg + 1].tick <= ev.tick) seg++;
      const s = tempoMap[seg];
      ev.timeSeconds = s.timeSeconds + ((ev.tick - s.tick) / timeDivision) * (s.microsecondsPerBeat / 1_000_000);
    }
  }
